# This pipeline is created like if retraining model 
# (It will only send the new model to the production like u r angain trying to trained new model !st it will compare with the existing one)
...
class ModelEvaluation:
    ...
    def initiate_model_evaluation(self) -> ModelEvaluationArtifact:
        try:
            ...
            if metric_info_artifact.index_number == 1: # means if trained model is accepted than index[1]  else index[o] for model is accepted  in line no 125
                model_evaluation_artifact = ModelEvaluationArtifact(evaluated_model_path=trained_model_file_path,
                                                                    is_model_accepted=True)
                self.update_evaluation_report(model_evaluation_artifact)
                logging.info(f"Model accepted. Model eval artifact {model_evaluation_artifact} created")
